...
<body>
  ...
  <div class="position-relative" style="margin-top: 20px">
    <?php 
      if(isset($_POST['id_area']) && !empty($_POST['desc'])){
          $areaTarefa = Area::getInstance()->areaTarefaEdit($_POST['id_area']);
        if (!$areaTarefa) {
              Area::getInstance()->areaUpdate($_POST['id_area'], $_POST['desc']);
              header('Location:../?p=areas');
        } else {
          echo '<div class="alert alert-danger" role="alert">Operação não permitida! Área está vinculada a uma tarefa em execução!</div>';
        }   
      }
    ?>
    <div class="container">
      <form class="col-sm-12 col-md-6" method="POST">
        ...
        <button type="submit" name="btn_update" class="btn btn-success">Salvar</button>
